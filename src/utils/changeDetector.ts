 ...
 */
export function getChanges<T extends mongoose.Document>(doc: T, newData: Record<string, any>): { field: string; oldValue: any; newValue: any }[] {
  const changes = [];
  const oldDoc = doc.toObject();

  for (const key in newData) {
    if (Object.prototype.hasOwnProperty.call(oldDoc, key) && newData[key] !== undefined) {
